...
export default function BurgerShowcase({ burger, activeIndex, isReady, isActive, isInteractive, finalSequenceActive, mobileMinHeight, onTrayGestureStart, onTrayGestureMove, onTrayGestureEnd, onBurgerTap, onTrayStep }) {
  ...
  return (
    <section id="tray" className={`showcase${isReady ? ' is-ready' : ''}${isActive ? ' is-active' : ''}${showMilasPreview ? ' has-milas-preview' : ''}${isLastBurger ? ' is-last-burger' : ''}`} style={{ '--mobile-showcase-min-height': mobileMinHeight }} aria-label="Burger menüsü">      <div className="showcase-panel">
